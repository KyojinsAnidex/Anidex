	...
	export async function load({ params }) {
		...
		async function proxygenreinfo() {
			...
			if (response.status === 200) {
				...
				alert('No Anime Of That Genre Found');
				return await response.json();
			} else {
		...
		async function fetchgenreinfo() {
			let temp = await proxygenreinfo();
		...
		async function fetchanimes() {
			animes = await fetchgenreinfo();
			// console.log(animes);
			for (let i = 0; i < animes.animeOfGenre.length; i++) {
				let animeid = animes.animeOfGenre[i].animeid;
				// console.log(animeid);
