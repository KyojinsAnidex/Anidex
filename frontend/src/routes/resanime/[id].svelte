...
<script>
	export let id;
	import { animesearch, animepics, state, curruser, eps, epanime,animeofinterest,userepratings,studio } from '../../stores/store';
	...
	{
		$studio=refanime.animestudio[i].studioname;
		console.log(studio);
	}
</script>
...
{#await refresh() then}
	<div class="relative flex h-full w-full bg-solarizedBase3 text-solarizedBase02">
		...
		<div class="h-full w-1/2">
			<div class="flex flex-col items-center justify-center w-full max-w-lg mx-auto">
				<AccordionFlush id="1">
					<h2 slot="header" class="text-xl font-medium">Synopsis</h2>
					<div slot="body">
				...
				{#if $state == 1}
					<AccordionFlush id="2">
						<h2 slot="header" class="text-xl font-medium">Add To Watchlist</h2>
						<div slot="body">
							{#if userrating!=0}
